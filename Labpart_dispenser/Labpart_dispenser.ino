...
//  Arduino Giga R1 WiFi + RA8875 800x480
//
//  Integrates:
...
int capDispenseBatch(int s, int count) {
  ...
  for (int i = 0; i < count; i++) {
    if (!capFeedOne(s)) { dispenserFlag[3 + s] = FLAG_REMOVE; break; }
    fed++;
  }
  if (fed > 0) {
...
bool capLoad(int s) {
  ...
  long pulses = 0; bool ok = true;
  while (!schmitt(d.sensorPin, d.blocked, d.hi, d.lo)) {
    pulseStep(d.stepPin, CAP_LOAD_DELAY_US);
    if (++pulses > CAP_LOAD_TIMEOUT) { ok = false; break; }
  }
  enRC(d.enBit, false);
...
//  RESISTOR LOGIC
// =================================================================
int angleToPulse(int a) { return map(a, 0, 180, SERVO_MIN, SERVO_MAX); }
void moveServoCh(uint8_t ch, int angle) { pwm.setPWM(ch, 0, angleToPulse(angle)); }
...
int resDispense(int s, int qty) {
  ...
  ResDispenser& d = res[s];
  int target = qty;
  if (d.needExtra) { target += 2; d.needExtra = false; }
  ...
  while (remaining > 0) {
    ...
    for (int i = 0; i < chunk; i++) {
      if (!resFeedOne(s)) { dispenserFlag[6 + s] = FLAG_REMOVE; break; }
      dispensed++;
    }
...
bool resLoad(int s) {
  ...
  enRC(d.enBit, false);
  if (ok) { dispenserFlag[6 + s] = FLAG_LOADED; d.needExtra = true; }
  else    { dispenserFlag[6 + s] = FLAG_REMOVE; }
  return ok;
}
...
void homeM1M2() {
  ...
  while (moved < HOME_MAX_STEPS) {
    ...
    digitalWrite(WM1_STEP, HIGH); digitalWrite(WM2_STEP, HIGH);
    delayMicroseconds(2);
    digitalWrite(WM1_STEP, LOW);  digitalWrite(WM2_STEP, LOW);
...
void wireGuillotine() {
  ...
  enW(WM2_EN_BIT, true);
  delay(5);
  stepMotorsSynced(WM1_DIR, WM1_STEP, WM2_DIR, WM2_STEP, 650, WIRE_STEP_DELAY_US);
  enW(WM1_EN_BIT, false);
  enW(WM2_EN_BIT, false);
  homeM1M2();
}
...
void updateLoadStatusFromSensors() {
  ...
  for (int s = 0; s < 3; s++) {
    ...
    wireUpdateStock(s);
    if (wireStock[s] != previousStock &&
        currentPage == PAGE_RELOAD && !popupVisible) {
      drawReloadButton(s);
    }
  }
}
...
void wireLoad(int s) {
  wireDispense(s, 4);       // token feed to verify strip present
  wireUpdateStock(s);
}
...
void closeWarningPopup() { popupVisible = false; drawInterface(); }
...
void loop() {
  ...
  if (pointInRect(x, y, 0, 0, SCREEN_WIDTH / 2, TAB_H)) {
    if (currentPage != PAGE_DISPENSE) { currentPage = PAGE_DISPENSE; drawInterface(); }
    return;
  }
  if (pointInRect(x, y, SCREEN_WIDTH / 2, 0, SCREEN_WIDTH / 2, TAB_H)) {
    if (currentPage != PAGE_RELOAD) { currentPage = PAGE_RELOAD; drawInterface(); }
    return;
  }
  ...
  for (int i = 0; i < NUM_PARTS; i++) {
    ...
    if (pointInRect(x, y, partMinusX(i), buttonY, BTN_W, BTN_H)) {
      if (partEnabled[i] && quantities[i] > 0) { quantities[i]--; redrawQuantity(i); }
      return;
    }
    if (pointInRect(x, y, partPlusX(i), buttonY, BTN_W, BTN_H)) {
      if (partEnabled[i] && quantities[i] < 9) { quantities[i]++; redrawQuantity(i); }
      return;
    }
  }
  ...
  if (pointInRect(x, y, kitPlusX, kitButtonY, KIT_BTN_W, KIT_BTN_H)) {
    if (numKits < 10) { numKits++; redrawKitQuantity(); }
    return;
  }
